...
--*/
{
    ...
    UNICODE_STRING required_extension = RTL_CONSTANT_STRING(L"didi");
...
--*/
{
    ...
    UNICODE_STRING required_extension = RTL_CONSTANT_STRING(L"didi"); // юникод-строка с названием расширения
    ...
    if (!NT_SUCCESS(status))
    {
    ...
    else if ( // проверка на совпадение расширением
        RtlEqualUnicodeString(
            &required_extension,
            &NameInfo->Extension, \
            FALSE)) {
        ...
        if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) {
            ...
            if (Data->Iopb->Parameters.Write.WriteBuffer) { //проверка на наличия чего-нибудь в буфере
                ...
                //создаем оюъект класса AES
                struct AES_ctx ctx; //определение структуры
                //инициализируем, присваиваем ключ и вектор
                ...
                for (int i = 0; i < 1024; i++) {
                    ((char*)Data->Iopb->Parameters.Write.WriteBuffer)[i] = hexarray[i];//записываем в буфер
                }
            }
        }
